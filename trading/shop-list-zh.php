                ...
                <div class="top-bar-sorter">
                    <div class="sorter-wrapper d-flex align-items-center">
                        ...
                        <select class="sorter wide" name="SortBy" id="SortBy" onchange="SortBy()">
                            <option value="best-selling">最佳销售</option>
                            <option value="title-ascending">按字母, A-Z</option>
                            <option value="title-descending">按字母, Z-A</option>
                            <option value="price-ascending">价格, 低到高</option>
                            <option value="price-descending">价格, 高到低</option>
                        </select>
                    </div>
                </div>
...
<script>
    ...
    function to(id) {
        window.location.href = 'shop-single-zh.php?id=' + id;
    }
    function pre_page() {
        if(page>1) {
            page_select(page - 1);
        ...
        $.ajax({
            ...
            success: function(data) {
                if(data!=0) {
                    ...
                    $('#myTabContent').html('<center>这里什么也没有</center>');
                }
            },
            error: function () {
                xtip.alert("error");
    ...
    function add_to_cart(iid, q) {
        $.ajax({
            ...
            error: function () {
                xtip.alert("请先登录!");
                window.location.href = '../user/login-zh.php';
            }
        });
    }
